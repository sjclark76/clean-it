...
// import { Booking, DayAvailability as AdminDayAvailability, TimeSlot as AdminTimeSlot } from '@/types/availability';
...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        // Basic Validation
        if (!bookingData.date || !bookingData.startTime || !bookingData.clientName || !bookingData.clientEmail || !bookingData.clientPhone || !bookingData.serviceType) {
        ...
        const availabilitiesColl = db.collection<AdminDayAvailability>(availabilitiesCollectionName);
        ...
        const newBooking: Omit<Booking, '_id' | 'createdAt' | 'status'> & { status: 'pending_confirmation', createdAt: Date } = {
        ...
        const dayAvailability = await availabilitiesColl.findOne({ date: bookingData.date });
        if (!dayAvailability) {
            return NextResponse.json({ message: "Availability for this date not found." }, { status: 404 });
        }

        const slotsToBlockTimes: string[] = [];
        for (let i = 0; i < 5; i++) { // 4 for service, 1 for buffer = 5 half-hour slots
        ...
        if (!stillAvailable) {
            return NextResponse.json({ message: "The selected time slot is no longer available. Please choose another time." }, { status: 409 }); // 409 Conflict
        }
        ...
        if (updateResult.modifiedCount === 0 && updateResult.matchedCount > 0) {
            ...
            console.warn(`Availability slots for booking ${bookingResult.insertedId} on ${bookingData.date} at ${bookingData.startTime} were not updated, though a match was found.`);
        } else if (updateResult.matchedCount === 0) {
            console.error(`Failed to find availability document to update for date: ${bookingData.date} after booking ${bookingResult.insertedId}`);
            // Potentially roll back booking or notify admin
        }


        return NextResponse.json({ message: "Booking request received successfully!", bookingId: bookingResult.insertedId }, { status: 201 });

    } catch (error) {
        console.error("Booking API Error:", error);
        return NextResponse.json({ message: "Failed to process booking request", error: (error as Error).message }, { status: 500 });
    } finally {
        if (mongoClient) {
...
export async function GET() {
    ...
    try {
        ...
        const upcomingBookings = await bookingsColl.find({
            date: { $gte: todayStr },
            status: { $ne: 'cancelled' } // Optionally filter out cancelled bookings
        }).sort({ date: 1, startTime: 1 }).toArray(); // Sort by date then start time
        ...
        return NextResponse.json(
            { message: "Failed to retrieve bookings", error: (error as Error).message },
            { status: 500 }
        );
    } finally {
        if (mongoClient) {
