...
import AvailabilitySelector from '@/components/AvailabilitySelector'; // Adjust path if needed
...
export default function BookingPage() {
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        if (!selectedBookingSlot) {
            setFormMessage({ type: 'error', text: "Please select an available date and time slot." });
            setIsLoading(false);
            return;
        }
        ...
        try {
            ...
            if (response.ok) {
                ...
                setFormMessage({ type: 'error', text: result.message || "Booking failed. Please try again." });
            }
        } catch (error) {
            console.error("Booking submission error:", error);
            setFormMessage({ type: 'error', text: "An unexpected error occurred during booking." });
        } finally {
            setIsLoading(false);
        }
    };
        ...
        <div className="min-h-screen bg-gray-100 text-gray-800 flex flex-col font-sans">
            <header className="bg-white/80 backdrop-blur-md shadow-md sticky top-0 z-50">
                <nav className="container mx-auto px-4 sm:px-6 py-4 flex justify-between items-center">
                    <a href="/" className="text-2xl font-bold text-purple-600 hover:text-purple-500 transition-colors duration-300 flex items-center">
                        <CalendarIcon className="w-7 h-7 mr-2" />
                        Jessiah's Car Cleaning
                    </a>
                    <a href="/" className="px-3 py-2 text-gray-700 hover:text-purple-600 transition-colors duration-300">
                        &larr; Back to Home
                    </a>
                </nav>
            </header>
                ...
                <div className="container mx-auto px-4 sm:px-6">
                    <div className="max-w-2xl mx-auto bg-white p-8 md:p-10 rounded-xl shadow-xl">
                        <div className="text-center mb-8">
                            ...
                            <h1 className="text-3xl sm:text-4xl font-bold text-purple-600 mb-2">Book Your Service</h1>
                            <p className="text-gray-600 text-lg">
                                Fill out the form below and select an available 2-hour slot to request an appointment.
                            </p>
                        </div>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div>
                                ...
                                <textarea
                                    id="notes" name="notes" rows={3} // Reduced rows for a more compact form
                                    className="w-full bg-gray-50 border-gray-300 text-gray-900 rounded-md p-3 focus:ring-purple-500 focus:border-purple-500"
                        ...
                        <p className="text-center text-gray-600 mt-8 text-sm">
                            Please note: This is a request. Your appointment is confirmed once Jessiah contacts you.
                        </p>
                    </div>
                </div>
            </main>
